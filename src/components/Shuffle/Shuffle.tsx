...
const Shuffle: React.FC<ShuffleProps> = ({
    ...
    () => {
      ...
      const build = () => {
        ...
        chars.forEach(ch => {
          ...
          if (colorFrom) (inner.style as React.CSSProperties & Record<string, string>).color = colorFrom;
      ...
      /* Re-arm the hover trigger after animation completes */
      const armHover = () => {
  ...
  const classes = useMemo(
    () => ['shuffle-parent', ready ? 'is-ready' : '', className].filter(Boolean).join(' '),
    [ready, className]
  );
